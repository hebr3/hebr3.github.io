...
◊(define-meta title "20% Project - ProjectEuler #29")
◊(define-meta published "2017-02-11")
...
We can use ◊b{Flatten} to compress the ◊${99×99} array into a single vector ◊${9,801} elements long.
We then use ◊b{DeleteDuplicates} to remove all of the duplicated elements from the vector.
And finish with ◊b{Length} to find the total number of elements left over.

◊figure["ProjectEuler29c.png"]

And technically we are done. ◊b{The answer to this ProjectEular problem is ◊${9,183}.}
...
◊figure["ProjectEuler29d.png"]

I created the following two tables in ◊link["https://en.wikipedia.org/wiki/LaTeX" "LaTeX"] to highlight how the powers of ◊${2} repeat at specific intervals in the ◊${4} and ◊${8} power tables. A similar pattern will occur for the powers of ◊${3, 5,} and ◊${7}.

Similarly the power table for ◊${6} will be repeated in the power table for ◊${36} and ◊${10} in the power table for ◊${100}.

The following Matrix and ArrayPlot were generated in Mathematica to show the repeated numbers in the table given above. Observe that ◊${2^6} occurs the most often in this table and is represented in black in the ◊b{ArrayPlot} to signify that it is a larger value.

◊figure["ProjectEuler29e.png"]
◊figure["ProjectEuler29f.png"]

Expanding the array to include all of the numbers we originally calculated we get the following ◊b{ArrayPlot}. Here we see that most of the values that will have repeats are clustered around the top of the array corresponding to a values between ◊${2} and ◊${10}.

◊figure["ProjectEuler29g.png"]
